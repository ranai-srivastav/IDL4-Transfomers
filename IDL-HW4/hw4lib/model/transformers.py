...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layers = nn.ModuleList(
            [SelfAttentionEncoderLayer(d_model, num_encoder_heads, d_ff_encoder, dropout) for i in range(num_encoder_layers)])

        # TODO: Create decoder layers
        # Use ModuleList to create a list of decoder layers
        self.dec_layers = nn.ModuleList(
            [CrossAttentionDecoderLayer(d_model, num_decoder_heads, d_ff_decoder, dropout) for i in range(num_decoder_layers)])
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        pad_mask_src = PadMask(x_enc, x_enc_lengths).to(device=padded_sources.device)
        ...
        return x_enc, pad_mask_src, running_att, {'log_probs': ctc_logits.swapaxes(0, 1), 'lengths': x_enc_lengths}
    ...
    def decode(
        ...
        pad_mask_tgt = None
        if target_lengths is not None:
            pad_mask_tgt = PadMask(padded_targets, target_lengths).to(device=padded_targets.device)
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # TODO: Pass through decoder layer
            x_dec, self_attn, cross_attn = self.dec_layers[i](x_dec,
                                                              encoder_output,
